...
# Faixa valida de cada coluna numerica (mesma regua dos campos individuais).
RANGES = {
    "Defasagem": (-4, 3), "fase_num": (0, 7), "IDA": (0.0, 10.0),
    "IEG": (0.0, 10.0), "IAA": (0.0, 10.0), "IPS": (2.5, 10.0),
    "IPV": (2.5, 10.0), "IPP": (0.1, 10.0),
}
...
with st.expander("Como interpretar o resultado"):
    _rt = meta.get("robustez_temporal", {})
    _linha_temporal = (
        f"- Robustez temporal (treina 2022–23, testa 2023–24, um ano nunca visto): "
        f"**ROC-AUC ≈ {_rt['auc_out_of_time']}**."
        if _rt.get("auc_out_of_time") else ""
    )
    st.markdown(f"""
...
- Validação principal (por aluno — nenhum estudante em treino e teste ao mesmo tempo): **ROC-AUC ≈ {meta.get('auc_cv', 0.87)}**.
{_linha_temporal}
...
with aba_lote:
    ...
    st.markdown(
        ...
        "A planilha usa o padrão do Excel brasileiro (separador `;`, decimal com vírgula). "
        "O **dicionário das colunas** explica cada campo e os valores aceitos."
    )
    ...
    dicionario = pd.DataFrame([
        ("RA", "Identificação do aluno (código interno)", "texto livre"),
        ("Defasagem", "Fase atual − fase ideal; negativo = atrasado", "inteiro de -4 a 3"),
        ("fase_num", "Fase atual na Associação (0 = Alfa … 7 = 3º EM)", "inteiro de 0 a 7"),
        ("IDA", "Desempenho acadêmico (média das notas)", "0 a 10"),
        ("IEG", "Engajamento", "0 a 10"),
        ("IAA", "Autoavaliação", "0 a 10"),
        ("IPS", "Psicossocial", "2,5 a 10 (piso da escala)"),
        ("IPV", "Ponto de virada", "2,5 a 10 (piso da escala)"),
        ("IPP", "Psicopedagógico", "0,1 a 10 (piso da escala)"),
        ("Genero_pad", "Gênero do aluno", "Feminino ou Masculino"),
        ("rede", "Rede de ensino", "Pública ou Particular"),
    ], columns=["coluna", "o_que_e", "valores_aceitos"])

    d1, d2 = st.columns(2)
    d1.download_button("Baixar modelo de planilha (CSV)",
                       exemplo.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig"),
                       "modelo_turma.csv", "text/csv")
    d2.download_button("Baixar dicionário das colunas (CSV)",
                       dicionario.to_csv(index=False, sep=";").encode("utf-8-sig"),
                       "dicionario_colunas.csv", "text/csv")

    arquivo = st.file_uploader("Planilha da turma (CSV)", type=["csv"])
    if arquivo is not None:
        try:
            bruto = arquivo.getvalue().decode("utf-8-sig", errors="replace")
            dados = pd.read_csv(io.StringIO(bruto), sep=None, engine="python")
        except Exception as e:
        ...
        # Numeros no formato brasileiro (virgula decimal) viram ponto antes da conversao.
        for c in NUM:
            if dados[c].dtype == object:
                dados[c] = dados[c].astype(str).str.strip().str.replace(",", ".", regex=False)
            dados[c] = pd.to_numeric(dados[c], errors="coerce")

        em_branco = dados[NUM + CAT].isna().any(axis=1)
        fora = pd.Series(False, index=dados.index)
        problemas = []
        for c, (lo, hi) in RANGES.items():
            v = ~dados[c].between(lo, hi) & dados[c].notna()
            if v.any():
                problemas.append(f"{c} fora de {lo}–{hi} em {int(v.sum())} linha(s)")
            fora |= v
        if em_branco.any():
            st.warning(f"{int(em_branco.sum())} linha(s) com campos em branco ficaram sem resultado.")
        if fora.any():
            st.warning("Valores fora da escala ficaram sem resultado — " + " · ".join(problemas) +
                       ". Consulte o dicionário das colunas.")

        X = dados[NUM + CAT].copy()
        validas = ~em_branco & ~fora
        ...
        st.download_button("Baixar resultado (CSV)",
                           ordenado.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig"),
