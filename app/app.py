...
# O default e um aluno em fase com indicadores medianos - o perfil que o modelo
# mais sinaliza, entao a tela ja abre mostrando para que serve a ferramenta.
LIMITES = {
    "Defasagem": (-4, 3, 0),
    "fase_num": (0, 7, 4),
    "IDA": (0.0, 10.0, 6.0),
...
# Paleta institucional, extraída do relatório de atividades 2024 da Passos Mágicos.
MARINHO = "#001860"
AZUL = "#0060A8"
AZUL_CLARO = "#78A8C0"
VERDE = "#4A8A3D"
...
[data-testid="stWidgetLabel"] p {{ color: {TINTA}; font-weight: 600; font-size: .9rem; }}
[data-testid="stSliderTickBar"] {{ color: {AZUL_CLARO}; }}
.rotulo {{
  font-size: .68rem; letter-spacing: .16em; text-transform: uppercase;
  color: {AZUL_CLARO}; font-weight: 600; margin-bottom: .1rem;
}}
.prob {{
...
.faixa-nota {{ color: {TINTA}; font-size: .9rem; margin-top: .5rem; }}
.acao {{
  border-left: 3px solid {AZUL}; padding: .35rem 0 .35rem .7rem;
  margin: .35rem 0; font-size: .92rem; color: {TINTA};
}}
...
def medidor(prob, corte_medio, corte_alto):
    """Barra com as três zonas de risco e a posição do aluno."""
    pos = min(max(prob, 0), 1) * 100
    m, a = corte_medio * 100, corte_alto * 100
    return f"""
<div style="margin:.2rem 0 1.1rem 0;">
  <div style="position:relative;height:14px;border-radius:2px;overflow:hidden;
              background:linear-gradient(to right,
                {VERDE} 0%, {VERDE} {m}%,
                {AMBAR} {m}%, {AMBAR} {a}%,
                {TERRACOTA} {a}%, {TERRACOTA} 100%);">
  </div>
  ...
  <div style="display:flex;justify-content:space-between;margin-top:.5rem;
              font-size:.68rem;letter-spacing:.12em;text-transform:uppercase;color:{AZUL_CLARO};">
    <span>0%</span><span>Média da rede {corte_medio:.0%}</span>
    <span>Dobro da média {corte_alto:.0%}</span><span>100%</span>
  </div>
</div>"""
...
<div style="border-left:6px solid {AZUL};padding-left:1rem;margin-bottom:.4rem;">
  <div class="rotulo">Associação Passos Mágicos · PEDE 2022–2024</div>
  <h1 style="margin:.1rem 0 .2rem 0;font-size:2.3rem;">Risco de defasagem</h1>
  <div style="color:{TINTA};font-size:1rem;max-width:70ch;">
    Probabilidade de o aluno agravar sua defasagem no próximo ciclo, a partir dos
    indicadores deste ano. Serve para priorizar acompanhamento antes da piora.
  </div>
</div>
""", unsafe_allow_html=True)
...
with aba_ind:
    ...
    # Resultado ao vivo: recalcula a cada ajuste dos controles, sem botao.
    ...
    with r1:
        ...
        st.markdown(
            f'<div style="margin-top:.7rem;"><span class="selo" '
            f'style="background:{cor_faixa(faixa)};">Risco {faixa.lower()}</span></div>',
            unsafe_allow_html=True)
    with r2:
        st.markdown('<div class="rotulo">Posição na escala de risco</div>', unsafe_allow_html=True)
        st.markdown(medidor(prob, CORTE_MEDIO, CORTE_ALTO), unsafe_allow_html=True)
        leitura = {
            "Alto": "Acompanhamento prioritário neste ciclo.",
            "Médio": "Monitoramento próximo ao longo do ciclo.",
            "Baixo": "Sem sinal de agravamento no momento.",
        }[faixa]
        st.markdown(f'<div class="faixa-nota">{leitura}</div>', unsafe_allow_html=True)

    st.markdown('<div class="rotulo" style="margin-top:1.4rem;">Onde agir</div>',
                unsafe_allow_html=True)
    if acoes:
        for ind, val, lim, acao in acoes:
            st.markdown(
                f'<div class="acao"><b>{ind} {val:.1f}</b> '
                f'<span style="color:#7A879E;">(padrão da rede: {lim:.1f})</span><br>{acao}</div>',
                unsafe_allow_html=True)
    elif faixa != "Baixo":
        st.markdown('<div class="acao">Nenhum indicador isolado está abaixo do padrão — '
                    'é a combinação deles que eleva o risco. Vale acompanhamento integral '
                    'e reavaliação no próximo ciclo.</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="acao">Todos os indicadores dentro do padrão da rede. '
                    'Manter o acompanhamento regular.</div>', unsafe_allow_html=True)
